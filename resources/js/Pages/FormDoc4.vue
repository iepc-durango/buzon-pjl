...
const submitForm = async () => {
  // Si el tipo es "Otro" y hay un valor en "especificar", usamos ese valor para el tipo
  if (formData.value.tipo === 'Otro' && formData.value.especificar) {
    formData.value.tipo = formData.value.especificar;
  }

  try {
    // Enviar los datos del formulario al backend para generar el documento PDF
    const response = await axios.post('/generate-document', formData.value, {
    ...
    // Crear un enlace para abrir el archivo PDF en una nueva pestaña
